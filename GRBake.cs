...
public class GRBake : MonoBehaviour
{
    ...
    public bool BakeGameObject(GameObject toBake)
    {
        ...
        {
            if (toBake.name != "layout" + gRally.g.CurrentStage.CurrentLayout.ID.ToString())
            {
    ...
    void bakeThisCar()
    {
        //extRootCar = cc.CarBody.transform.FindChild("Ext").gameObject;
        intRootCar = GRSimData.Car.CarBody.transform.Find("Int").gameObject;

        intRootCar.name = "intOriginal";
        var newIntRootCar = new GameObject("Int");
        newIntRootCar.transform.parent = intRootCar.transform.parent;

        PrepareList();
        BakeCarGameObject(intRootCar, newIntRootCar);
        CreateCar(newIntRootCar);
    }

    void bakeThis()
    {
        GRElapsedTimer.TimerStart();
        ...
        foreach (var item in SceneManager.GetSceneByName("layout" + gRally.g.CurrentStage.CurrentLayout.ID.ToString()).GetRootGameObjects())
        {
            if (item.activeSelf)
            {
                BakeGameObject(item);
        ...
        DevConsole.Console.Log(string.Format("Bake done in {0}[ms]", GRElapsedTimer.TimerStop()), Color.yellow);
    }
    ...
    private void bakeTerrainThis()
    {
        ...
        DevConsole.Console.Log(string.Format("Bake Terrain done in {0}[ms]", GRElapsedTimer.TimerStop()), Color.yellow);
    }
